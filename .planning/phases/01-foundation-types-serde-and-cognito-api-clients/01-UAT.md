...
number: 1
name: AuthFactorType.webAuthn Enum Value
expected: |
  In `packages/amplify_core/lib/src/types/auth/sign_in/auth_factor_type.dart`, the `webAuthn` enum value exists with a `@JsonValue('WEB_AUTHN')` annotation. The old TODO/cadivus comment is removed.
